...
//! Returns a matrix of connection coefficients for Kerr metric.
//! 
//! Trying to optimize a little bit; by Wenda
//
//! (!) NOTE: Christoffel tensor Gamma^i_(jk) is symmetric in lower two indices (j,k).
//! This function only evaluates components of the tensor, where j<k and
//! multiplies the value of these coeficients by a factor of two.
...
//! @result Connection coeficients are returned in `G` parameter.
{
    ...
    double S  = r2 + a2c2;
    double S3 = S*S*S;
	double one_over_S = 1. / S;
	double one_over_D = 1. / D;
	double one_over_S3 = 1. / S3;
	double one_over_DS = one_over_S * one_over_D;
	double one_over_R = 1. / R;
	double one_over_DR = one_over_R * one_over_D;
    ...
    G[0][0][1] = -4. *(a2 + r2)*(a2 - twor2 + a2cc) * one_over_DR;
    G[0][0][2] = -16. * a2rcs * one_over_R;
    G[0][1][3] = 4. * as2 * (a4 - 3. * a2r2 - 6. * r4 + a2cc * (a2 - r2)) * one_over_DR;
    G[0][2][3] = -G[0][0][2] * as2;

    G[1][0][0] = -D*(a2c2-r2) * one_over_S3;
    G[1][0][3] = 2.0 * (as2 * D * (a2c2-r2)) * one_over_S3;
    G[1][1][1] = (r*(a2 - r) + a2c2 * (1. - r)) * one_over_DS;
    G[1][1][2] = -2.0 * (a2cs) * one_over_S;
    G[1][2][2] = -r*D * one_over_S;
    G[1][3][3] = -D*s2*(2. * a2c2 *r3 + r5 + a4*c2*s2 + c2*c2*a4*r - a2r2 * s2) * one_over_S3; //-D*s2*(c2*(2.*a2*r*(a2 + r2) + a4*(1. - r)*s2) + r*(-a4 + r2*r2 + a2*(a2 - r)*s2))/S3;

    G[2][0][0] = -2.* a2rcs * one_over_S3;
    G[2][0][3] = 4. * ar * cs*(a2 + r2) * one_over_S3;
    G[2][1][1] = a2cs * one_over_DS;
    G[2][1][2] = twor * one_over_S;
    G[2][2][2] = -G[2][1][1]*D;
    G[2][3][3] = -0.125 * cs*(3.*a2*a4 + 10.*a4*r + 11.*a4*r2 + 16. * a2r3 +
            16.*a2 * r4 + 8.*r3*r3 + four_a2cc *(a2 + twor2)*D +
            a4CC * D) * one_over_S3;

    G[3][0][1] = 2.0 * a*(r2 - a2c2)/(D*S*S);
    G[3][0][2] = -16. * ar * m_over_s * one_over_R;
    G[3][1][3] = (a4 + 3.*a4*r - 12. * a2r2 + 8. * a2r3 -
            16. * r4 + 8.*r5 + four_a2cc * r * (twor2 -r + a2) -
            a4CC * (1. - r)) * one_over_DR;
    G[3][2][3] = ((3.*a4 + 8.*a2*r + 8. * a2r2 + 8. * r4 +
            four_a2cc * (twor2 - twor + a2) + a4CC)* m_over_s) * one_over_R;
...
//! @result Changes the components of vector `V`.
{
    ...
    if (m) {
        ...
        c = V0*V0*m->g00;
        alpha = sim5max(-b/a+sqrt(b*b-a*c)/a, -b/a-sqrt(b*b-a*c)/a);
    } else {
