...
double rf(double x, double y, double z)
{
	...
	if ((sim5min(sim5min(x,y),z) < 0.0) || (sim5min(sim5min(x+y,x+z),y+z) < rfTINY) || (sim5max(sim5max(x,y),z) > rfBIG)) {
        #ifndef CUDA
		warning("%e/%e/%e\n", x,y,z);
	...
	do {
		...
		dely=(ave-yt)/ave;
		delz=(ave-zt)/ave;
	} while (sim5max(sim5max(fabs(delx),fabs(dely)),fabs(delz)) > ERRTOL);
...
double rd(double x, double y, double z) {
	...
	if ((sim5min(x,y) < 0.0) || (sim5min(x+y,z) < rdTINY) || (sim5max(sim5max(x,y),z) > rdBIG)) {
        #ifndef CUDA
...
double rj(double x, double y, double z, double p) {
	...
	if ((sim5min(sim5min(x,y),z) < 0.0) || (sim5min(sim5min(x+y,x+z),sim5min(y+z,fabs(p))) < rjTINY) ||
	  (sim5max(sim5max(x,y),sim5max(z,fabs(p))) > rjBIG)) {
        #ifndef CUDA
	...
	if (p > 0.0) {
		...
		xt=sim5min(sim5min(x,y),z);
		zt=sim5max(sim5max(x,y),z);
		yt=x+y+z-xt-zt;
		a=1.0/(yt-p);
		b=a*(zt-yt)*(yt-xt);
		pt=yt+b;
	...
	do {
		...
		delz=(ave-zt)/ave;
		delp=(ave-pt)/ave;
	} while (sim5max(sim5max(fabs(delx),fabs(dely)),sim5max(fabs(delz),fabs(delp))) > ERRTOL);
