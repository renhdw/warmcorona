...
# 基本配置
# =========================
OUT_DIR = os.path.expanduser("~/data/monk/plot/warmcorona/test/2026.03.11/compare_KN/sphere")

SPHERE_BIN = "/home/hdw/data/monk/monk_for_rhy/bin/sphere"
...
def run_one_case(case_dir, te, tau, stype):
    sphere_dir = os.path.join(case_dir, "sphere")
    calspec_dir = os.path.join(case_dir, "calspec")
    os.makedirs(sphere_dir, exist_ok=True)
    os.makedirs(calspec_dir, exist_ok=True)

    params_out = os.path.join(sphere_dir, "params.txt")
    update_params_file(BASE_PARAMS_FILE, params_out, te, tau, stype)

    run_command(SPHERE_BIN, cwd=sphere_dir)
    print(f"[OK] sphere finished: te={te}, tau={tau}, stype={stype}")

    run_command(f"{CALSPEC_BIN} ../sphere/ {CALSPEC_PARAMETER}", cwd=calspec_dir)
