...
def create_folders(te_lower_bound, te_upper_bound, tau_lower_bound, tau_upper_bound, te_parts, tau_parts,
    ...
    base_file_path = '/data/hdw/data/monk/plot/warmcorona/test/test_smooth_10_7/data' # 修改了路径，变为linux格式，本行修改log存储位置
...
#    chose_log_file_path = r'\\wsl.localhost\Ubuntu-22.04\home\hdw\data\monk\plot\warmcorona\test\test_smooth\data\chose.log'
    ...
    # 参数位置基础文件
    base_file_path_linux = r'/data/hdw/data/monk/plot/warmcorona/test/test_smooth_10_7'
...
#    xspec_flux_log_file_path = r'\\wsl.localhost\Ubuntu-22.04\home\hdw\data\monk\plot\warmcorona\test\test_smooth\data\Xspec_flux.log'
    ...
    tasks = []
    with ProcessPoolExecutor(max_workers=6) as executor: #用于并行计算
...
def main():
    parser = argparse.ArgumentParser(description="控制模拟参数")

    # 基础模式：测试 or 非测试
    parser.add_argument('--test', type=int, choices=[0, 1], required=True,
                        help='是否是测试阶段（1是，0否）')
    ...
    parser.add_argument('--calspec', nargs=3, type=float,
                        help='calspec参数（三个数字，空格分隔）')

    # 通用参数
    parser.add_argument('--select', choices=['row', 'column', 'random', 'all'], required=True,
                        help='选点方式：row/column/random/all')
    parser.add_argument('--ratio', type=float, default=1.0,
                        help='数据点占比（0~1），仅 row/column/random 有效')
    parser.add_argument('--seed', type=int, default=None,
                        help='随机种子，仅 random 有效')
