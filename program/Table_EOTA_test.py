...
def output_param_settings(param_list, filename):
    """
    将模型参数信息写入文本文件，方便查看
    """
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for i, p in enumerate(param_list):
            f.write(f"参数 {i+1}: 名称 = {p.getName()}\n")
            f.write(f"  初始值 = {p.getInitialValue()}\n")
            f.write(f"  Delta = {p.getDelta()}\n")
            f.write(f"  范围 = [{p.getMinimum()}, {p.getMaximum()}]\n")
            f.write(f"  Bottom = {p.getBottom()}, Top = {p.getTop()}\n")
            unique_vals = p.getTabulatedValues()
            f.write(f"  唯一值数量 = {len(unique_vals)}\n")
            f.write(f"  唯一值数组 = {unique_vals}\n")
            f.write("-"*40 + "\n")
...
def main():
    ...
    # 归一化步骤：预先遍历所有flux，找全局最大flux值
    max_flux_value = 0
    flux_array_list = []
    for idx, row in spec_df.iterrows():
        flux = np.array(row[1:], dtype=float)
        flux_array_list.append(flux)
        max_flux_value = max(max_flux_value, flux.max())

    # 新增打印全局最大flux
    print(f"全局最大flux值: {max_flux_value:.6e}")
    logger.info(f"全体光谱中的最大flux值为: {max_flux_value}")

    tbl = table()
    tbl.setModelName("DiskTest")
    tbl.setModelUnits(" ")
    ...
    output_param_settings(param_objs, param_setting_file)
    logger.info(f"参数设置信息已写入: {param_setting_file}")
    ...
    for idx, row in spec_df.iterrows():
        ...
        flux_norm = flux / max_flux_value if max_flux_value > 0 else flux
    ...
    outfile = os.path.join(base_dir, "disk_test.mod")
