...
if missing:
    raise FileNotFoundError(f"These .mod files are missing (slab must exist too):\n" +
                            "\n".join(missing))
...
pdf_name = f"cmp_warm_vs_nth_Te{str(kTe_nth).replace('.','p')}_G{str(gamma).replace('.','p')}_z{str(z_value).replace('.','p')}_{timestamp}.pdf"
...
for i, (name, kind, path, tag, _, qdp_w) in enumerate(warm_jobs):
    ...
    print(f"[{name}] kind={kind} | scale={scale:.4g} | "
...
# ====== PDF page 1: Spectra + fitted straight lines (per-spectrum Emin) ======
param_text = (
    f"nthComp: kTe={kTe_nth:.3f} keV, Gamma(input)={gamma:.3f}, kTbb={kTbb_nth:.4f}, z={z_value}\n"
...
with PdfPages(pdf_path) as pdf:
    ...
    for r in results:
        ...
        plt.loglog(r["E"], r["F"], color=r["color"], lw=lw,
                   label=f"{r['label']} | Gamma_fit={r['Gamma_range']:.3f}")
    # Overlay fitted straight lines over [Emin_fit, E_MAX_FIT]
    for r in results:
        if not np.isfinite(r["m"]) or not np.isfinite(r["b"]) or not np.isfinite(r["Emin_fit"]):
            continue
    ...
    plt.title(f"warmcom vs nthComp (same Te, tau); dashed = fit over [Emin_data, {E_MAX_FIT:.1f}] keV")
    ...
    for r in results[1:]:
        ...
        ax1.set_xlabel("Energy [keV]"); ax1.set_ylabel("Residual"); ax2.set_ylabel("Ratio")
        lines, labels = ax1.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        ax1.legend(lines + lines2, labels + labels2, loc="best", fontsize=8)
        info = (f"{r['label']} | Gamma_fit[{r['Emin_fit']:.3f}-{E_MAX_FIT:.3f}]={r['Gamma_range']:.3f}\n"
                f"{param_text}")
